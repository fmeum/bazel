...
final class DigestScratchBuffers {
   ...
   *
   * <p>Large enough to keep the number of {@code read()} syscalls low on filesystems where they are
   * expensive (network filesystems, FUSE), small enough that retaining one per slot is free. Beyond
   * this size, larger buffers stop paying for themselves: the digest computation becomes bound by
   * the hash function rather than by the read loop.
   */
  private static final int BUFFER_SIZE = 32 * 1024;

  /** Smallest and largest permissible number of slots. Both must be powers of two. */
  private static final int MIN_SLOTS = 16;
   ...
   *
   * <p>Sized generously relative to the number of cores so that threads concurrently digesting
   * files rarely collide, while bounding retained memory to a few megabytes.
   */
  private static final int SLOTS =
      Math.clamp(
          Integer.highestOneBit(4 * Runtime.getRuntime().availableProcessors() - 1) * 2,
          MIN_SLOTS,
          MAX_SLOTS);
