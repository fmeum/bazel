...
public class DigestUtils {
   ...
   *
   * <p>Obtaining it requires a {@code stat()}, which is why callers that already know a file's
   * metadata should pass it in: for a file whose digest is cached, that syscall would otherwise be
   * the dominant cost of the lookup. Skyframe records these very properties for every file it knows
   * about (see {@code FileContentsProxy}), so they are usually available without touching the
   * filesystem again.
   */
