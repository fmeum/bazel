...
public class RepositoryUtils {
   ...
   *
   * @param safeForLocalCacheReuse whether it is safe to reuse the replanted repo from a different
   *     physical location (e.g. a shared repo contents cache). This is the case when there are no
   *     cross-repo symlinks or symlinks into the main repo, or on Windows with symlinks enabled
   *     (where symlinks resolve using logical rather than physical paths).
   * @param hasSymlinkIntoMainRepo whether the repo contains a symlink pointing into the main repo
   */
  public record ReplantedSymlinks(boolean safeForLocalCacheReuse, boolean hasSymlinkIntoMainRepo) {}
  ...
  public static ReplantedSymlinks replantSymlinks(
      Path repoDir,
      Path workspace,
    ...
    return new ReplantedSymlinks(portableSymlinksOnly, hasSymlinkIntoMainRepo);
  }
}
