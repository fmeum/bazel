...
public final class ModCommand implements BlazeCommand {
  ...
  private BlazeCommandResult execInternal(CommandEnvironment env, OptionsParsingResult options) {
    ...
    if (subcommand.equals(ModSubcommand.DUMP_REPO_MAPPING)) {
      // Collect all extension repos generated by extensions that have already been evaluated, but
      // do not force any evaluations.
      var memoizingEvaluator = env.getSkyframeExecutor().getEvaluator();
      ImmutableMultimap.Builder<ModuleExtensionId, String> knownExtensionRepos =
          ImmutableMultimap.builder();
      for (ModuleExtensionId extensionId : depGraphValue.getExtensionUsagesTable().rowKeySet()) {
        try {
          var extensionEvalValue =
              (SingleExtensionEvalValue)
                  memoizingEvaluator.getExistingValue(SingleExtensionEvalValue.key(extensionId));
          if (extensionEvalValue != null) {
            knownExtensionRepos.putAll(
                extensionId, extensionEvalValue.getGeneratedRepoSpecs().keySet());
          }
        } catch (InterruptedException e) {
  ...
  public static void dumpRepoMapping(
      BazelDepGraphValue depGraphValue,
      ImmutableMultimap<ModuleExtensionId, String> knownExtensionRepos,
      Writer writer) throws IOException {
    var json = new JsonObject();

    var moduleRepos = new JsonObject();
    for (ModuleKey modules : depGraphValue.getDepGraph().keySet()) {
      var repoMapping = new JsonObject();
      for (Entry<String, RepositoryName> e :
          depGraphValue.getFullRepoMapping(modules).entries().entrySet()) {
        repoMapping.addProperty(e.getKey(), e.getValue().getName());
      }
      moduleRepos.add(modules.getCanonicalRepoName().getName(), repoMapping);
    }
    json.add("module_repos", moduleRepos);

    var extensionRepos = new JsonObject();
    for (var e : knownExtensionRepos.asMap().entrySet()) {
      var extension = new JsonObject();
      extension.addProperty("module_repo", e.getKey().getBzlFileLabel().getRepository().getName());

      var generatedRepos = new JsonArray();
      e.getValue().forEach(generatedRepos::add);
      extension.add("generated_repos", generatedRepos);

      var prefix = depGraphValue.getExtensionUniqueNames().get(e.getKey());
      extensionRepos.add(prefix + "~", extension);
    }
    json.add("extension_repos", extensionRepos);
