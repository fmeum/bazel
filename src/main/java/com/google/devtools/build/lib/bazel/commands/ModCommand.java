...
public final class ModCommand implements BlazeCommand {
  ...
  private BlazeCommandResult execInternal(CommandEnvironment env, OptionsParsingResult options) {
    ...
    if (subcommand.equals(ModSubcommand.DUMP_REPO_MAPPING)) {
      ...
      for (String arg : args) {
        try {
          ...
          env.getReporter().handle(Event.error(null, "Invalid repository name: " + e.getMessage()));
          return reportAndCreateFailureResult(
              env, "Invalid repository name", Code.INVALID_ARGUMENTS);
        }
      }
    }
    ImmutableList<SkyKey> repositoryMappingKeys = repositoryMappingKeysBuilder.build();

    BazelDepGraphValue depGraphValue;
    BazelModuleInspectorValue moduleInspector;
    List<RepositoryMappingValue> repositoryMappingValues;
    ...
    try {
      ...
      if (subcommand.equals(ModSubcommand.DUMP_REPO_MAPPING)) {
        keys.addAll(repositoryMappingKeys);
      } else {
