...
class BazelLockfileTest(test_base.TestBase):
  ...
  def testDeletedFactsRegeneratedByUpdateMode(self):
    """Facts deleted from the workspace lockfile are regenerated by UPDATE mode.
    ...
    # UPDATE mode should rerun the extension and regenerate the facts even
    # though the extension's result is cached and none of its inputs changed.
    self.RunBazel(['build', '@hello//:all', '--lockfile_mode=update'])
