...
class RemoteRepoContentsCacheTest(test_base.TestBase):
  ...
  def testMaterializedRepoDoesNotInvalidateLoadedPackages(self):
    ...
    # Bazel serve its files from disk instead of from memory. The file state
    # values computed before and after differ in their representation, but not
    # in their contents, which must not invalidate the packages and .bzl files
    # loaded from the repo in the next command.
    self.ScratchFile(
        'MODULE.bazel',
        [
            'repo = use_repo_rule("//:repo.bzl", "repo")',
    ...
    _, _, stderr = self.RunBazel(['build', '--nobuild', '//main:use_data'])
    self.assertNotIn('JUST FETCHED', '\n'.join(stderr))
    self.assertNotIn('will be fetched again', '\n'.join(stderr))
    self.assertIn(
        'Analyzed target //main:use_data (0 packages loaded, 0 targets'
        ' configured).',
        '\n'.join(stderr),
