...
public class DiskCacheClientTest {
  ...
  public void downloadBlob_toPathBackedStream_whenDeletedAfterRefresh_throwsCacheNotFoundException()
    ...
    try {
      ...
      Path casPath = populateCas(raceClient, digest, "contents".getBytes(UTF_8));
      raceFs.deleteOnNextMtimeUpdate(casPath);
      // A path-backed output stream makes DiskCacheClient copy the file instead of streaming it,
      // which is the code path taken when staging action outputs into bazel-out/_tmp/actions/remote
      // and the one that surfaces the missing entry as a FileNotFoundException.
